...
function messageView(m, viewerId) {
  ...
  const recipient = userById(m.recipientId);
  return {
    id: m.id,
    ...
    type: m.type,
    // One-time text stays hidden from the recipient until they open it via /open-once.
    text: m.deletedAt ? null : (m.type === "text" && m.oneTime && viewerId === m.recipientId ? null : m.text),
...
app.post("/api/messages/:id/delete", auth, (req,res) => {
  const m = db.messages.find(x => x.id === req.params.id);
  if (!m || m.senderId !== req.user.id) return sendError(res,"NOT_ALLOWED","Only the sender can delete this message.",403);
  m.deletedAt = now();
  saveDB();
  io.to(`user:${m.senderId}`).emit("message:deleted",{id:m.id});
  io.to(`user:${m.recipientId}`).emit("message:deleted",{id:m.id});
  res.json({ok:true});
});
